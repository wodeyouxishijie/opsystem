...
public abstract class AbstractTomcatContainer implements TomcatContainer {
	 protected Log logger = LogFactory.getLog(getClass());
	    ...
	    public File getConfigFile(Context ctx) {
	        String configFilePath = ctx.getConfigFile().getFile();
	    ...
	    protected List getFilterMappings(FilterMap fmap, String dm, String filterClass) {
	        ...
	        if (fmap.getURLPatterns().length > 0)
	        {
	            fm.setUrl(fmap.getURLPatterns()[0]);
	        }
	        if (fmap.getServletNames().length > 0)
	        {
	            fm.setServletName(fmap.getServletNames()[0]);
	        }
	    ...
	    public abstract void stop(String name) throws Exception ;
	    
	    public abstract String getConfigBase();
	    
	    public abstract File getAppBase();
}
